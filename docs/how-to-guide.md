...
- [Example Usage](#example-usage)<br>
  ...
  - [Saving and loading processed measurements](#saving-and-loading-processed-measurements)<br>
...
### Saving and loading processed measurements

Processing a folder of many files — and in particular adaptive downsampling via the processing parameters — can take a while. When the same processed data is reused across several scripts, the resulting ```Measurement``` can be saved to a single, named file and reloaded instantly with ```save()``` and ```load()```.
...
tam = Measurement(folder=path_to_data, processparams=processparams)

# save the processed object to a named file
tam.save("processed_run.pkl")
```

In a later script the object is restored in one step, ready for plotting and analysis. The whole object is stored, so the processed data, the information table, any added metadata and the ```processparams``` are all preserved.

```python
from calocem import Measurement

tam = Measurement.load("processed_run.pkl")
tam.plot()
```

The saved file is a convenience cache, not an archive: the raw source files remain the source of truth. A file written by an incompatible CaloCem version raises an error on load and can always be regenerated by re-running from the original folder. The ```.pkl``` extension is recommended to keep the pickle format explicit.
